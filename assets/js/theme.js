	...
	_createClass(DomNode, [
		...
		},
		{
			...
			value: function camelize(str) {
				...
				return ""
					.concat(text.substr(0, 1).toLowerCase())
					.concat(text.substr(1));
			},
		},
	]);
	...
	if (bottomBars.length) {
		bottomBars.forEach(function (bar) {
			...
			var toggleBottomBar = function toggleBottomBar() {
				if (
					window.matchMedia(
						"(max-width: ".concat(utils.breakpoints[breakPoint], "px)")
					).matches
				) {
			...
			var toggleBottomBarOnNavbarCollapse =
				function toggleBottomBarOnNavbarCollapse(el) {
			...
			value: function clickBulkCheckbox() {
				...
				this.element.on("click", function () {
					if (_this.element.attr("indeterminate") === "indeterminate") {
						...
						_this.replacedElement.removeClass(
							_this.option.displayNoneClassName
						);
				...
				this.bulkSelectRows.forEach(function (el) {
					...
					rowCheck.on("click", function () {
						if (_this2.element.attr("indeterminate") !== "indeterminate") {
							...
							_this2.replacedElement.addClass(
								_this2.option.displayNoneClassName
							);
						}

						if (
							_toConsumableArray(_this2.bulkSelectRows).every(function (
								element
							) {
...
var chatInit = function chatInit() {
	...
	document.querySelectorAll(Selector.CHAT_CONTACT).forEach(function (el) {
		el.addEventListener(Events.CLICK, function (e) {
			...
			window.innerWidth < 768 &&
				...
				($chatSidebar.style.left = "-100%"); // Remove unread-message class when read
	...
	$chatContact.forEach(function (el) {
		el.addEventListener(Events.SHOWN_BS_TAB, function () {
			...
			var TargetChatArea = document.querySelector(
...
var draggableInit = function draggableInit() {
	...
	var columnContainers = document.querySelectorAll(
	...
	if (columnContainers.length) {
		...
		var sortable = new window.Draggable.Sortable(columnContainers, {
			...
			scrollable: {
				draggable: Selectors.KANBAN_ITEM,
				scrollableElements: [].concat(_toConsumableArray(columnContainers), [
					container,
				]),
			},
...
var dropdownMenuInit = function dropdownMenuInit() {
	...
	if (window.is.ios()) {
		...
		document
			.querySelectorAll(Selector.TABLE_RESPONSIVE)
			.forEach(function (table) {
				table.addEventListener(Event.SHOWN_BS_DROPDOWN, function (e) {
					var t = e.currentTarget;

					if (t.scrollWidth > t.clientWidth) {
						t.style.paddingBottom = "".concat(
		...
		iconList.addEventListener("click", function (e) {
			...
			if (el.tagName === "INPUT") {
				...
				iconCopiedToast.querySelector(".toast-body").innerHTML =
					'<span class="fw-black">Copied:</span> <code>'.concat(
						el.value,
						"</code>"
					);
...
var inputmaskInit = function inputmaskInit() {
	if (window.Inputmask) {
		...
		elements.forEach(function (item) {
			...
			var inputmask = new window.Inputmask(_objectSpread({}, maskOptions)).mask(
				item
			);
			return inputmask;
		});
...
var kanbanInit = function kanbanInit() {
	...
	var collapseDismissButtons = document.querySelectorAll(
...
var togglePaginationButtonDisable = function togglePaginationButtonDisable(
	button,
	disabled
) {
	...
	if (window.List) {
		...
		if (lists.length) {
			lists.forEach(function (el) {
				...
				var paginationButtonNext = el.querySelector(
					'[data-list-pagination="next"]'
				);
				var paginationButtonPrev = el.querySelector(
				...
				if (options.pagination) {
					el.querySelector(".pagination").addEventListener(
						"click",
						function (e) {
...
var scrollInit = function scrollInit() {
	var dropdownElements = Array.from(
		document.querySelectorAll("[data-hide-on-body-scroll]")
	);
...
var searchInit = function searchInit() {
	...
	document
		.querySelectorAll(Selectors.DROPDOWN_TOGGLE)
...
var initialDomSetup = function initialDomSetup(element) {
	if (!element) return;
	var dataUrlDom = element.querySelector(
	...
	if (window.tinymce) {
		...
		if (tinymces.length) {
			...
			window.tinymce.init({
				...
				content_style: ".mce-content-body { color: ".concat(
...
var treeviewInit = function treeviewInit() {
	...
	if (treeviews.length) {
		treeviews.forEach(function (treeview) {
			...
			var striped =
				options === null || options === void 0 ? void 0 : options.striped;
			var select =
			...
			var collapseElementList = Array.from(
				treeview.querySelectorAll(Selector.TREEVIEW_LIST)
			);
			var collapseListItem = Array.from(
				...
				if (select) {
					...
					inputElement.addEventListener(Events.CHANGE, function (e) {
						var childInputElements = Array.from(
							treeview
								.querySelector("#".concat(collapseId))
...
var wizardInit = function wizardInit() {
	...
	wizards.forEach(function (wizard) {
		...
		var inputConfirmPassword = wizard.querySelector(
		...
		addEventForm &&
			addEventForm.addEventListener(Events.SUBMIT, function (e) {
				...
				calendar.addEvent({
					...
					className:
						allDay.checked && label.value
							? "bg-".concat(label.value, "-subtle")
							: "",
...
var managementCalendarInit = function managementCalendarInit() {
	...
	if (managementCalendar) {
		...
		var managementCalendarEvents = document.getElementById(
			calendarData === null || calendarData === void 0
				? void 0
				: calendarData.events
		);
		var addEventForm = document.querySelector(Selectors.ADD_EVENT_FORM);
		var addEventModal = document.querySelector(Selectors.ADD_EVENT_MODAL);
		var eventDetailsModal = document.querySelector(
			Selectors.EVENT_DETAILS_MODAL
		);

		var updateTitle = function updateTitle(title) {
			var selectTitle = document.getElementById(
				calendarData === null || calendarData === void 0
					? void 0
					: calendarData.title
			);
		...
		var updateDay = function updateDay(day) {
			...
			var selectDay = document.getElementById(
				calendarData === null || calendarData === void 0
					? void 0
					: calendarData.day
			);
		...
		var eventManagementInfo = document.querySelectorAll(
...
var productShareDoughnutInit = function productShareDoughnutInit() {
	var marketShareDoughnutElement = document.getElementById(
		"marketShareDoughnut"
	);
...
var activeUsersChartReportInit = function activeUsersChartReportInit() {
	var $echartsActiveUsersChart = document.querySelector(
...
var assignmentScoresInit = function assignmentScoresInit() {
	var $echartAssignmentScores = document.querySelector(
...
var avgEnrollmentRateInit = function avgEnrollmentRateInit() {
	var $echartsLineAvgEnrollmentLms = document.querySelector(
	...
	if ($echartsLineAvgEnrollmentLms) {
		...
		var onSaleCourseRate = document.querySelector(
			"#".concat(userOptions.optionOne)
		);
		var regularPaidCourseRate = document.querySelector(
			"#".concat(userOptions.optionTwo)
		);
			...
			return {
				...
				series: [
					...
					},
					{
						...
						type: "line",
						data: [
							1700, 1200, 500, 700, 1500, 1100, 700, 1100, 2600, 2050, 1050,
							600, 700,
						],
						lineStyle: {
...
var bandwidthSavedInit = function bandwidthSavedInit() {
	var $echartsBandwidthSaved = document.querySelector(
...
var bounceRateChartInit = function bounceRateChartInit() {
	...
	if ($echartsBounceRateChart) {
				...
				series: [
					{
						type: "line",
						data: [
							40, 37, 42, 44, 36, 39, 37, 43, 38, 35, 43, 39, 42, 36, 37, 36,
							42, 44, 34, 41, 37, 41, 40, 40, 43, 34, 41, 35, 44, 41, 40,
						],
						showSymbol: false,
						symbol: "circle",
						itemStyle: {
		...
		echartSetOption(chart, userOptions, getDefaultOptions);
		var selectMenu = document.querySelector(
...
var browsedCoursesInit = function browsedCoursesInit() {
	var $echartsBrowsedCourses = document.querySelector(
	...
	if ($echartsBrowsedCourses) {
		...
		var newCourseBrowsedEl = document.querySelector(
			"#".concat(userOptions.optionOne)
		);
		var paidCourseBrowsedEl = document.querySelector(
			"#".concat(userOptions.optionTwo)
		);
				...
				series: [
					{
						...
						z: -1,
						data: [
							600, 832, 901, 934, 1290, 1330, 1320, 1250, 1190, 1345, 1009,
							1320, 600, 832, 901, 934, 1290, 1330, 1320, 1250, 1190, 1345,
							1009, 1320,
						],
						itemStyle: {
							emphasis: {
				...
				xAxis: {
					...
					axisLabel: {
						...
						formatter: function formatter(value) {
							...
							return ""
								.concat(date.getDate(), " ")
								.concat(months[date.getMonth()], " , 21");
						},
						margin: 20,
					},
				},
...
var courseEnrollmentsInit = function courseEnrollmentsInit() {
	var $echartBarCourseEnrollments = document.querySelector(
...
var echartsCustomerSatisfactionInit =
	function echartsCustomerSatisfactionInit() {
		var $echartCustomerSatisfaction = document.querySelector(
...
var dealStorageFunnelInit = function dealStorageFunnelInit() {
	var $echartDealStorageFunnel = document.querySelector(
...
var echartsDistributionOfPerformanceInit =
	function echartsDistributionOfPerformanceInit() {
		var $distributionOfPerformance = document.querySelector(
			...
			if (utils.getData(item, "echart-responsive")) {
				if (
					!(
						...
						window.getComputedStyle(item.closest(".tab-pane")).display ===
							"none"
					)
				) {
		...
		var getDefaultOptions = function getDefaultOptions() {
			return {
				tooltip: {
					...
					formatter: function formatter(params) {
						return ""
							.concat(params[0].axisValue, " - ")
							.concat(params[0].value, " USD");
					},
					textStyle: {
						fontWeight: 500,
						fontSize: 12,
...
var locationBySessionInit = function locationBySessionInit() {
	var $locationBySessionMap = document.querySelector(
		".echart-location-by-session-map"
	);
	var data = [
		{
			name: "Afghanistan",
	...
	if ($locationBySessionMap) {
				...
				tooltip: {
					...
					formatter: function formatter(params) {
						...
						return "<strong>"
							...
							.concat(
								(
									(((_params$data2 = params.data) === null ||
									_params$data2 === void 0
										? void 0
										: _params$data2.value) /
										total) *
									100
		...
		(_document$querySelect3 = document.querySelector(
			".location-by-session-map-zoom"
		)) === null || _document$querySelect3 === void 0
			? void 0
...
var marketingExpensesInit = function marketingExpensesInit() {
	var $echartsMarketingExp = document.querySelector(
...
var echartsNumberOfTicketsInit = function echartsNumberOfTicketsInit() {
	...
	if ($numberOfTickets) {
		...
		var numberOfTicketsLegend = document.querySelectorAll(
...
var reportForThisWeekInit = function reportForThisWeekInit() {
	...
	var $echartBarReportForThisWeek = document.querySelector(
	...
	if ($echartBarReportForThisWeek) {
		...
		var legendThisWeek = document.getElementById(
			selectChart === null || selectChart === void 0
				? void 0
				: selectChart.option1
		);
...
var returningCustomerRateInit = function returningCustomerRateInit() {
	var ECHART_LINE_RETURNING_CUSTOMER_RATE =
...
var salesByPosLocationInit = function salesByPosLocationInit() {
	var ECHART_RADAR_SALES_BY_POS_LOCATION =
		".echart-radar-sales-by-pos-location";
	var $echartsRadarSalesByPosLocation = document.querySelector(
...
var echartsSatisfactionSurveyInit = function echartsSatisfactionSurveyInit() {
	var $satisfactionSurvey = document.querySelector(
...
var sessionByCountryMapInit = function sessionByCountryMapInit() {
	var $sessionByCountryMap = document.querySelector(
	...
	if ($sessionByCountryMap) {
			...
			return {
				tooltip: {
					...
					formatter: function formatter(params) {
						...
						return "<strong>"
							.concat(
								(_params$data3 = params.data) === null ||
									_params$data3 === void 0
									? void 0
									: _params$data3.name,
								" :</strong> "
							)
							.concat(
								(
									(((_params$data4 = params.data) === null ||
									_params$data4 === void 0
										? void 0
										: _params$data4.value) /
										total) *
									100
		...
		echartSetOption(chart, userOptions, getDefaultOptions);
		(_document$querySelect5 = document.querySelector(
			".session-by-country-map-reset"
...
var echartTicketPriority = function echartTicketPriority() {
	var $paginationBtnNext = document.querySelector(
		'[data-list-pagination-chart="next"]'
	);
	var $paginationBtnPrev = document.querySelector(
...
var echartsUnresolvedTicketsInit = function echartsUnresolvedTicketsInit() {
	...
	if ($unresolvedTickets) {
		...
		var unresolvedTicketsLegend = document.querySelectorAll(
			"[data-unresolved-tickets]"
		);
...
var userByLocationInit = function userByLocationInit() {
	var $userByLocationMap = document.querySelector(
	...
	if ($userByLocationMap) {
			...
			return {
				tooltip: {
					...
					formatter: function formatter(params) {
						...
						return "<strong>"
							.concat(
								(_params$data5 = params.data) === null ||
									_params$data5 === void 0
									? void 0
									: _params$data5.name,
								" :</strong> "
							)
							.concat(
								(_params$data6 = params.data) === null ||
									_params$data6 === void 0
									? void 0
									: _params$data6.value
							);
					},
		...
		echartSetOption(chart, userOptions, getDefaultOptions);
		var zoomLevel = 1;
		(_document$querySelect6 = document.querySelector(
			".user-by-location-map-zoom"
		)) === null || _document$querySelect6 === void 0
			? void 0
		...
		(_document$querySelect7 = document.querySelector(
			".user-by-location-map-zoomOut"
